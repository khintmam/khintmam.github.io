...
app.directive("feedItem", function () {
...
}).controller("FeedController", ['$scope', 'commentService', 'authService', 'feedService', 'ngDialog', '$location',
    function ($scope, commentService, authService, feedService, ngDialog, $location) {
        //init
    ...
    $scope.LikeFeed = function () {
        feedService.likeFeed($scope.feed.id).then(function (results) {
    ...
    $scope.SadFeed = function () {
        feedService.sadFeed($scope.feed.id).then(function (results) {
            if (results.data == -2) alert(-2);
            else
    ...
    $scope.ViewImageByFeed = function () {
        ngDialog.closeAll();
        $location.path('/viewimagebyfeed/' + $scope.feed.id);
    }
